...
Verify with Router `/v1/completions` against `Qwen72B_INT8`. An aggregated co-located calibration was **2251** tok/s at TP=4 (`CHUNK_SIZE=3071`); TP=8 on the same node measured **2438** tok/s (~+8%). The P/D-path figure is **1465** tok/s (NIXL + sidecar) and is not valid here.
