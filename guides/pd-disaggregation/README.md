...
Verify with Router `/v1/completions` or `/v1/chat/completions` against `Qwen72B_INT8`. Confirm decode logs show an external KV transfer rather than decode-only recompute. Re-measure `peakPrefillThroughput` in the router values for this model and card before performance work. An aggregated Biren 166M / Qwen2.5-72B GPTQ-Int8 / TP=4 calibration through the full P/D path was **1465** tok/s (`CHUNK_SIZE=3071`); the default `pd-disaggregation.values.yaml` figure is for gpt-oss-120b on NVIDIA and is not valid here.
